...
// Route ids that share one registry entry: the six chart pages all ship from the
// single "chart" component, so they read its description.
...
for await (const page of glob("src/(examples)/*/page.html")) {
  ...
  for (const { label, body } of blocks) {
    ...
    entries.push(`  ${JSON.stringify(slug(label))}: { label: ${JSON.stringify(label)}, code: ${jsString(code)}, html: ${jsString(html)} },`);
  }
...
const described = routes
  .sort()
  .map(id => [id, byName.get(REGISTRY_ALIASES[id] ?? id)?.description])
...
await writeFile(
  ...
  [
    ...
    ...Object.entries(REGISTRY_ALIASES).map(([id, name]) => `  ${JSON.stringify(id)}: ${JSON.stringify(name)},`),
    "};",
    "",
  ].join("\n"),
);
